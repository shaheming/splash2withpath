...
typedef struct	gmem
	...
	long (start);			/* Barrier for startup sync.	     */
	long (pidlock);		/* Lock to increment pid.	     */
	long (ridlock);		/* Lock to increment rid.	     */
	long (memlock);		/* Lock for memory manager.	     */
	long (wplock);	/* Locks for shared work pools.      */
    UINT par_start_time;
